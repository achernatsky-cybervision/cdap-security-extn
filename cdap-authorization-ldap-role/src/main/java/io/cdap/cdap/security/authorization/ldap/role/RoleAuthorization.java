...
public class RoleAuthorization implements AccessController {
  ...
  private LDAPSearcher createLDAPSearcher(AuthorizationContext context) {
    ...
    if (Objects.isNull(searchConfig.getLookUpBindDN()) && Objects.isNull(searchConfig.getLookUpBindPassword())) {
      LOG.info("Properties '{}' and '{}' were empty in deployment", RoleAuthorizationConstants.LOOK_UP_BIND_DN,
               RoleAuthorizationConstants.LOOK_UP_BIND_PASSWORD);
    ...
    return new LDAPSearcher(searchConfig);
  }

  private boolean isVisible(EntityId entityId, PrincipalPermissions principalPermissions) {
    EntityType entityType = entityId.getEntityType();
    Permission getPermission = StandardPermission.GET;

    return isAccessible(entityType, entityId, getPermission, principalPermissions);
  }
